...
注意点としては、Azure Portalで指定したサービスプリンシパルは、オプションのパラメータとなり、必須ではありません。AROクラスターの作成コマンド「az aro create」を実行するときに、サービスプリンシパルをパラメータとして指定しなかった場合は、自動的に、各AROクラスター用のサービスプリンシパルが作成されます。
